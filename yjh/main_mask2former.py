...
import sys
sys.path.append('/data/yjh_files/code/Mask2Former-Simplify-master')
import torch
...
def build(cfg):
    ...
    if cfg.dataset == 'EndoVis2017':
        writer_dir = os.path.join(
            '/data/yjh_files/data/Endovis2017/result_new/',
            cfg.task,str(cfg.fold)
        )
        writer = SummaryWriter(writer_dir, flush_secs=15)

        all_paths = sorted(glob.glob(os.path.join(cfg.root, "*.h5")))

        test_seqs = fold_seq[cfg.fold]
        ...
        print(f"#Test files: {len(test_paths_temp)}")

    if cfg.dataset == 'EndoVis2018':
        writer_dir = os.path.join('/data/yjh_files/data/Endovis2018/result_new/', cfg.task)
        if not os.path.exists(writer_dir):
            os.makedirs(writer_dir)
            # os.mkdir(writer_dir)只能创建单级目录
        writer = SummaryWriter(writer_dir, flush_secs=15)
        train_path='/data/yjh_files/data/Endovis2018/data_h5/train/'
        test_path='/data/yjh_files/data/Endovis2018/data_h5/test/'
        for i in os.listdir(train_path):
            if i.replace('.h5','') in remove_list or i.replace('.h5','') in remove_list_train1:
                continue
            name=i.replace('.h5','')
            train_paths_temp.append(os.path.join(train_path,name))
        test_paths=sorted(glob.glob(os.path.join(test_path,'*')))
        for i in test_paths:
    ...
    if cfg.tracker:
        ...
        seg_model.train(train_paths_temp, test_paths_temp, cfg.epochs,writer,cfg.fold)
...
def get_args():
    ...
    parser.add_argument('--inferonly', default=True, type=bool )  #是否推理
    ...
    parser.add_argument("--dataset", type=str, default="EndoVis2017", choices=['EndoVis2017','EndoVis2018']) #使用数据集
    parser.add_argument("--seed", type=int, default=50)
    parser.add_argument("--fold", type=int, default=3) #使用第几个fold
    parser.add_argument("--task", type=str, default="endovis_2017_final_vis") #任务名（保存文件夹名）
    parser.add_argument("--root", type=str, default="/data/yjh_files/data/Endovis2017/data_h5/train/") #训练数据集根目录
...
def main():
    ...
    torch.cuda.manual_seed_all(seed)
       ...
       # 为了更高的可复现性
    torch.backends.cudnn.deterministic = True  # 保证操作可复现
    torch.backends.cudnn.benchmark = False     # 禁止基准优化，确保确定性结果
    # torch.use_deterministic_algorithms(True)

    build(config)
if __name__ == "__main__":
    main()
# /data/yjh_files/miniconda3/envs/yjh/bin/python /data/yjh_files/code/Mask2Former-Simplify-master/yjh/main_mask2former.py
...
'''
for i in 0 1 2 3; do
  /data/yjh_files/miniconda3/envs/yjh/bin/python  \
  /data/yjh_files/code/Mask2Former-Simplify-master/yjh/main_mask2former.py \
  --fold $i
done

for i in 0 1; do
  seed=$((50 + i))
  task="endovis_2018_baseline_swins_21_${i}"
  /data/yjh_files/miniconda3/envs/yjh/bin/python /data/yjh_files/code/Mask2Former-Simplify-master/yjh/main_mask2former.py --seed "$seed" --task "$task"
done
'''
# tensorboard --logdir /data/yjh_files/data/Endovis2018/result_new/endovis_2018_baseline_swins_21_0/2/ --port 6009
