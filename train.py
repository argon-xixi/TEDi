...
def load_config(args):
    ...
    cfg.MODEL.DENOISING_DECODER = FvCfgNode()
    cfg.MODEL.DENOISING_DECODER.DECODER_LAYERS = 6
...
def run_training(cfg):
    ...
    from maskformer_train import tedi
    ...
    try:
        tedi(cfg).train(train_paths, test_paths, cfg.epochs, writer, cfg.fold)
    finally:
        writer.close()
...
def run_efficiency_analysis(cfg):
    ...
    from modeling.MaskFormerModel_tcd_mse import MaskFormerModel_denoising_memorybank

    device = torch.device(cfg.eff_device if torch.cuda.is_available() else "cpu")
    model = MaskFormerModel_denoising_memorybank(cfg).to(device).eval()
