...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets,matcher_outputs=None):
        ...
        targets = self._get_targets_baseline(targets)
        ...
        indices_gpu =  [tuple(value.to(outputs['pred_logits'].device)  for value in idx) for idx in indice_label]
        indices_dict.update({"indices_main_layer":indices_gpu})
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                indice_label = [(m,t["labels"][J]) for t, (m, J) in zip(targets, indices)]
                indices_gpu =  [tuple(value.to(outputs['pred_logits'].device)  for value in idx) for idx in indice_label]
                indices_dict.update({"indices_aux_layer_"+str(i): indices_gpu})
        return losses,indices_dict
