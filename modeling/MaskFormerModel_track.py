...
from .dvis.video_dvis_modules_new import ReferringTracker, TemporalRefiner
import sys
...
class MaskFormerModel_track(nn.Module):
    ...
    def forward(self, inputs,targets_ori,istrain,name_idx,epoch, i, name):
        ...
        with torch.no_grad():
            ...
            ###
            pred_masks = image_outputs['pred_masks'].detach()
 
            # 生成 binary_mask (b, q, t, h, w)
            binary_mask = (pred_masks > 0.0)  # (b, q, t, h, w)

            # 获取 mask 高度和宽度
            b, q, t, h, w = binary_mask.shape
            half_width = w // 2  # 中间的分割位置
      
            left_sum  = binary_mask[..., :half_width].sum(dim=(-2, -1))   # (b,q,t)
            right_sum = binary_mask[..., half_width:].sum(dim=(-2, -1))   # (b,q,t)

            # 判断大部分在左边还是右边，如果在左边为0，右边为1
            position_vector = (right_sum >= left_sum).long()   # b,q,t 右边大于左边为1，否则为0
            ####
    ...
    #     mask_features = image_outputs['mask_features'].clone().detach()
        ...
        position_vector=0
        outputs_ori, indices = self.tracker(position_vector,frame_embds, mask_features, return_indices=True, resume=self.keep) #outoutput是tracker的输出
        ...
        if istrain:
            append_epoch_jsonl("/data/yjh_files/code/Mask2Former-Simplify-master/yjh/indice/train/{}".format(self.cfg.task), epoch, i, name[name_idx], indice_dict)
        else:
            append_epoch_jsonl("/data/yjh_files/code/Mask2Former-Simplify-master/yjh/indice/test/{}".format(self.cfg.task), epoch, i, name[name_idx], indice_dict)
        ...
        if istrain:
            return outputs_reshape,losses,indice_dict
        else:
            ...
            return outputs_ori,losses,indice_dict
