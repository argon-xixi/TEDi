...
# from yjh.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
...
class HungarianMatcher_bbox(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_mask: float = 1, cost_dice: float = 1,cost_bbox: float = 1,cost_giou: float = 1, num_points: int = 0):
        ...
        self.cost_instance= 0.5

        assert cost_class != 0 or cost_mask != 0 or cost_dice != 0 or cost_bbox != 0 or cost_giou != 0, "all costs cant be 0"
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            out_bbox = outputs["pred_bbox"][b] #[num_queries,4]

            tgt_ids = targets[b]["labels"] # [1,2,3, ……]            
            tgt_mask = targets[b]["masks"].to(out_mask)
            tgt_bbox = targets[b]["boxes"].to(out_bbox) # [num_total_targets, 4]
            #(1,512,512) [c, h, w] c = len(tgt_ids)
            ...
            cost_instance=self.cost_bbox * cost_bbox+self.cost_giou * cost_giou
            # Final cost matrix
            C = (
