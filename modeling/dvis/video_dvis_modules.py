...
class ReferringTracker(torch.nn.Module):
    ...
    def forward(self,position_vector, frame_embeds, mask_features, resume=False, return_indices=False):
        ...
        """
        frame_embeds = frame_embeds.permute(2, 0, 3, 1)  # t, b, q, c
        # frame_embeds = frame_embeds.unsqueeze(3)  # t, q, b, 1, c
        n_frame, bs,n_q, _ = frame_embeds.size()
        outputs = []
        ret_indices = []

        for i in range(n_frame):
            ms_output = []
            single_frame_embeds = frame_embeds[i]  # b q c 
            ...
            if i == 0 and resume is False:
                ...
                for j in range(self.num_layers):
                    if j == 0:
                        ms_output.append(single_frame_embeds) #加入当前帧的分割结果
                        indices = self.match_embds(self.last_frame_embeds, single_frame_embeds)
                        self.last_frame_embeds = single_frame_embeds[torch.arange(bs, device=indices.device).unsqueeze(1),indices]
                        ret_indices.append(indices)
                        output = self.transformer_cross_attention_layers[j](    #当前帧的分割结果作为q，上一帧的的去噪后结果作为k和v
                            single_frame_embeds[torch.arange(bs, device=indices.device).unsqueeze(1),indices], single_frame_embeds[torch.arange(bs, device=indices.device).unsqueeze(1),indices], self.last_outputs[-1], #是上一帧最后的输出self.last_outputs[-1]
                            memory_mask=None,
                            memory_key_padding_mask=None,
                            pos=None, query_pos=None
                        )
                        ...
                        output = self.transformer_cross_attention_layers[j](
                            ms_output[-1], ms_output[-1], self.last_outputs[-1],
                            memory_mask=None,
                            memory_key_padding_mask=None,
                            pos=None, query_pos=None
                        )
                        ...
                        ms_output.append(output)
            ms_output = torch.stack(ms_output, dim=0)  # (1 + layers, b, q, c)
            self.last_outputs = ms_output 
            outputs.append(ms_output[1:])
        outputs = torch.stack(outputs, dim=0)  # (t, l, b, q, c)
        ...
        out = {
           ...
           'pred_embds': outputs[:, -1].permute(1, 3, 0, 2)  # (b, c, t, q)
        }
        if return_indices:
    ...
    def match_embds(self, ref_embds_batch, cur_embds_batch):
        #  embeds (b,q, c)
        b,q,c= ref_embds_batch.shape
        indice_batch=[]
        for i in range(b):
            ref_embds, cur_embds = ref_embds_batch.detach()[i, :, :], cur_embds_batch.detach()[i, :, :]
            ...
            indice_batch.append(torch.from_numpy(indices[1]))
        col_indices = torch.stack(indice_batch, dim=0)
...
#             return out
    ...
    def prediction(self, outputs, mask_features):
        ...
        decoder_output = decoder_output.permute(1, 2, 0, 3, 4)  # (l, b, t, q, c)
