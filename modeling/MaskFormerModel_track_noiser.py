...
from .dvis.video_dvis_modules_noiser import ReferringTracker, TemporalRefiner
import sys
...
class MaskFormerModel_track(nn.Module):
    ...
    def build_tracker(self, cfg):
        # building criterion
        # 兼容：若配置中未显式设置 NOISE_MODE/NOISE_RATIO，则退化为无噪声（保持原逻辑不变）
        noise_mode = getattr(cfg.MODEL.TRACKER, "NOISE_MODE", "none")
        noise_ratio = getattr(cfg.MODEL.TRACKER, "NOISE_RATIO", 0.0)
