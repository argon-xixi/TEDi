...
class MaskFormer_bbox():
    ...
    def load_model(self, pretrain_weights):
        ...
        # state_dict = torch.load(pretrain_weights, map_location='self.device')
    ...
    @torch.no_grad()                   
    def evaluate(self, eval_loder,epoch,writer):
        ...
        with torch.no_grad():
            for i, (data, targets,feat_sam,name) in enumerate(eval_loder):
                ...
                if self.cfg.bina==True:
                    ...
                    image_left= data.to(device=self.device, non_blocking=True)
                    outputs = self.model(image_left,feat_sam)
                losses = self.criterion(outputs, target,bboxes,cls)
                weight_dict = self.criterion.weight_dict           
                loss_ce = 0.0
                ...
                reduced_loss_bbox = reduce_value(loss_bbox, average=True)
                reduced_loss_giou = reduce_value(loss_giou, average=True)
                
                losses_list_val.append(reduced_loss.item())
                loss_ce_list_val.append(reduced_loss_ce.item())
                loss_dice_list_val.append(reduced_loss_dice.item())
                loss_mask_list_val.append(reduced_loss_mask.item())
                loss_bbox_list_val.append(reduced_loss_bbox.item())
                loss_giou_list_val.append(reduced_loss_giou.item())
                ...
                if self.cfg.inferonly:
                    pred_masks_val = self.semantic_inference_topk(mask_cls_results, mask_pred_results,1)
                else:
                ...
                gts=  self.instance_mask2semantic_mask_batch (target,cls)  
                # print(gts.max())
                targets_list.append(gts)     
              
             # 合并本地进程的数据
            preds_local = torch.cat(preds_list, dim=0)
            targets_local = torch.cat(targets_list, dim=0)
            
            # 跨进程收集数据
            world_size = dist.get_world_size()
            rank = dist.get_rank()
            
            gathered_preds = [torch.zeros_like(preds_local) for _ in range(world_size)] if rank == 0 else None
            gathered_targets = [torch.zeros_like(targets_local) for _ in range(world_size)] if rank == 0 else None
            
            dist.gather(preds_local, gather_list=gathered_preds, dst=0)
            dist.gather(targets_local, gather_list=gathered_targets, dst=0)
            
            # 主进程计算全局指标
            if rank == 0:
                # 合并所有进程的数据
                preds_all = torch.cat(gathered_preds, dim=0)
                targets_all = torch.cat(gathered_targets, dim=0)
                
                # # 计算指标
                # mm={}
                # for K in preds_all[20].detach().cpu().numpy().flat:
                #     if K not in mm.keys():
                #         mm[K]=1
                #     else:
                #         mm[K]+=1
                # print(mm)
                # mm={}
                # for K in targets_all[20].detach().cpu().numpy().flat:
                #     if K not in mm.keys():
                #         mm[K]=1
                #     else:
                #         mm[K]+=1
