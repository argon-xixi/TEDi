...
# ===== Ablation switch =====
# - memorybank+track: MaskFormerModel_track_memorybank
# - memorybank-only : MaskFormerModel_memorybank_only
...
# from modeling.MaskFormerModel_track_noiser import MaskFormerModel_track
# bina 分支用到的模型存在额外依赖（如 flow_warp）。
# 为避免在非 bina 实验时报 ImportError，这里做延迟/容错导入。
try:
    from modeling.MaskFormerModel_bina import MaskFormerModel_bina  # type: ignore
except Exception:
    MaskFormerModel_bina = None
...
def seed_everything(seed: int = 42, deterministic: bool = True):
    ...
    注意：某些 CUDA 算子天生非确定性；若你追求 *严格* 可复现，
    还需要在运行脚本前设置环境变量：

        export CUBLAS_WORKSPACE_CONFIG=:4096:8

    并尽量避免 DataParallel / 多进程带来的浮点规约顺序差异。
    """
    import os
    import random
    import numpy as np
    ...
    def train_epoch(self,data_loader,epoch,writer,warmup_only=False):
        ...
        for i, (data, target,name) in enumerate(data_loader):  
            ...
            if self.cfg.bina:
                ...
                # name_idx=torch.tensor([i for i in range(len(name))]).to(device=self.device, non_blocking=True)
            ...
            # loss = w_ce * loss_ce + loss_dice + loss_mask
            ...
            # pred_logits :(b, t, q, h, w)  pred_masks: (b, t, q, c)
            # 下面这些仅用于日志，不参与反向传播；放在 no_grad 块中可减少 autograd 开销
            with torch.no_grad():
                outputs_post = self.post_processing(outputs)
                mask_cls_results = outputs_post["pred_logits"] #(b, q, h, w)
                mask_pred_results = outputs_post["pred_masks"] #(b, q, c)
    ...
    def evaluate(self, eval_loader, epoch, writer):
        ...
        with torch.no_grad():
            ...
            for i, (data, target,name) in enumerate(eval_loader):
                ...
                #         pic_name = name[idx]
                #         if not  "seq_2_frame" in pic_name :
                #             continue
                #         # print(indice_dict['indices_main_layer'][idx])
