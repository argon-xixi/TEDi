...
class DVIS_online(MinVIS):
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training and self.window_inference:
            ...
            with torch.no_grad():
                features = self.backbone(images.tensor)
                image_outputs = self.sem_seg_head(features) #当前帧的预测结果
            ...
            outputs, indices = self.tracker(frame_embds, mask_features, return_indices=True, resume=self.keep) #tracker的输出
            image_outputs = self.reset_image_output_order(image_outputs, indices) #image_output被重新排序
    ...
    def reset_image_output_order(self, output, indices):
        """
        通过匹配索引对预测的分割掩膜（masks）和预测的logits进行重新排序
