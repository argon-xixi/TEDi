...
class MaskFormer():
    ...
    def train_epoch(self,data_loader, epoch,writer):
        data_loader.sampler.set_epoch(epoch)#在每个 epoch 开始时调用 set_epoch() 方法，然后再创建 DataLoader 迭代器，以使 shuffle 操作能够在多个 epoch 中正常工作
        ...
        for i, (data, target,feat_sam,name) in enumerate(data_loader):  
            ...
            loss = 1.5*loss_ce + 0.75*loss_dice + loss_mask
