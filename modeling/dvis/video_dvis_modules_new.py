...
def fix_all_true_attn_mask(attn_mask: torch.Tensor):
    """
    如果某一行（最后一维）全为 True，则整行取反
    """
    assert attn_mask.dtype == torch.bool

    # 找到哪些 query 行是“全 True”
    all_true = attn_mask.all(dim=-1, keepdim=True)  # shape: (..., 1)

    # 对这些行进行翻转
    fixed_mask = torch.where(all_true, ~attn_mask, attn_mask)
...
class ReferringTracker(torch.nn.Module):
    ...
    def forward(self, position_vector,frame_embeds, mask_features, resume=False, return_indices=False):
        """
        ...
        try:
            for i in range(n_frame):
                ...
                current_position_prior = position_vector[:,i]  # q
                
                # the first frame of a video
                if i == 0 and resume is False:
                    self._clear_memory()
                    self.last_frame_embeds = single_frame_embeds
                    attn_mask = ~(current_position_prior[:, None] == current_position_prior).bool()
                    # print("attn_mask:",attn_mask.sum())
                    self_attn_mask = ~(current_position_prior[:, None] == current_position_prior).bool() #false代表
                    # print("self_attn_mask:",self_attn_mask.sum())
                    # attn_mask=torch.full((n_q, n_q), 1.0, device=frame_embeds.device)
                    self.last_position_prior = current_position_prior
                    for j in range(self.num_layers):
                        if j == 0:
                            ms_output.append(single_frame_embeds)
                            ret_indices.append(self.match_embds(single_frame_embeds, single_frame_embeds)) #仅在第一层时排序
                            output = self.transformer_cross_attention_layers[j](
                                single_frame_embeds, single_frame_embeds, single_frame_embeds, #此时自己对自己做交叉注意力
                                memory_mask=fix_all_true_attn_mask(attn_mask), #false代表可以注意力连接，true代表屏蔽
                                memory_key_padding_mask=None,
                                pos=None, query_pos=None
                            )
                            output = self.transformer_self_attention_layers[j](
                                output, tgt_mask=fix_all_true_attn_mask(self_attn_mask),
                            ...
                            output = self.transformer_cross_attention_layers[j](
                                ...
                                memory_mask=fix_all_true_attn_mask(attn_mask),
                                memory_key_padding_mask=None,
                                pos=None, query_pos=None
                            )
                            output = self.transformer_self_attention_layers[j](
                    ...
                    for j in range(self.num_layers):
                        if j == 0:
                            ms_output.append(single_frame_embeds)
                            indices = self.match_embds(self.last_frame_embeds, single_frame_embeds) #当前帧与上一帧进行匹配排序（上一帧已经经过重排）
                            # current_position_prior_resetorder = current_position_prior[indices]
                            attn_mask = ~(self.last_position_prior[:, None] == current_position_prior).bool()
                            # print("attn_mask:",attn_mask.sum())
                            self_attn_mask = ~(current_position_prior[:, None] == current_position_prior).bool()
                            # print("self_attn_mask:",self_attn_mask.sum())
                            self.last_position_prior = current_position_prior[indices]
                            self.last_frame_embeds = single_frame_embeds[indices] #对当前帧进行重排,并用于下一张图的比较
                            ret_indices.append(indices)
                            output = self.transformer_cross_attention_layers[j](  #用重排后的第一帧作为初始化
                            ...
                            output = self.transformer_cross_attention_layers[j](
                                ms_output[-1], self.last_outputs[-1], single_frame_embeds,  #res,q,k
                                memory_mask=fix_all_true_attn_mask(attn_mask),
                                memory_key_padding_mask=None,
                                pos=None, query_pos=None
                            )
...
class TemporalRefiner(torch.nn.Module):
    ...
    def windows_prediction(self, outputs, mask_features, windows=5):
        ...
        for i in range(iters):
            ...
            outputs_masks.append(outputs_mask.to(torch.float32))
        outputs_classes = torch.cat(outputs_classes, dim=2)
        outputs_classes = self.pred_class(outputs_classes)
        return outputs_classes.to(torch.float32), torch.cat(outputs_masks, dim=3)
