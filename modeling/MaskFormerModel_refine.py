...
from .dvis.video_dvis_modules import ReferringTracker, TemporalRefiner
import sys
...
class MaskFormerModel_refine(nn.Module):
    def __init__(self, cfg):
        ...
        self.refiner = self.build_refiner(cfg)
    ...
    def forward(self, inputs,targets_ori,istrain):
        self.keep = False
        self.backbone.eval()
        ...
        # do temporal refine
        outputs_ori = self.refiner(instance_embeds, frame_embds_, mask_features)
    ...
    def reset_image_output_order(self, output, indices):
        ...
        output['pred_masks'][0] = output['pred_masks'][0][indices, frame_indices].transpose(0, 1) #[indices, frame_indices]表示的实际是原始的输出结果在[3,64]维度上每一个值重排后对应的索引
