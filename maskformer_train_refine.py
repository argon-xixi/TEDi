...
from modeling.MaskFormerModel_refine import MaskFormerModel_refine
...
class MaskFormer_refine():
    def __init__(self, cfg):
        ...
        self.model = MaskFormerModel_refine(cfg)
    ...
    def train_epoch(self,data_loader,epoch,writer):
        ...
        for i, (data, target,feat_sam,name) in enumerate(data_loader):  
            ...
            if self.cfg.bina:
                ...
                t0 = time.time()
                outputs,losses = self.model(inputs,target,self.istrain)  
                # traini
                # ng loop
