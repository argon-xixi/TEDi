...
from modeling.MaskFormerModel_sam_new import MaskFormerModel_sam_new
...
class MaskFormer_sam_new():
    def __init__(self, cfg):
        ...
        self.model = MaskFormerModel_sam_new(cfg)
    ...
    def train_epoch(self,data_loader, epoch,writer):
        ...
        for i, (data, target,feat_sam,name) in enumerate(data_loader):  
            ...
            losses = self.criterion(outputs, target,matcher_outputs=None)
            torch.cuda.synchronize()
    ...
    def evaluate(self, eval_loader, epoch, writer):
        ...
        with torch.no_grad():
            for i, (data, target,feat_sam,name) in enumerate(eval_loader):
                ...
                # mask_pred_results=outputs['pred_masks'] =einops.rearrange(outputs['pred_masks'], '(b t) q h w -> b t q h w', t=3)[:,1,:,:,:]
                # mask_cls_results=outputs['pred_logits'] =einops.rearrange(outputs['pred_logits'], '(b t) q c -> b t q c', t=3)[:,1,:,:]
                # gt_mask=gt_mask[:,1,:,:]
                instance=False
