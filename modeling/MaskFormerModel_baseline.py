...
from .transformer_decoder.mask2former_transformer_decoder import MultiScaleMaskedTransformerDecoder #没有sam
import einops
...
class MaskFormerHead(nn.Module):
    ...
    def predictor_init(self, cfg):
        ...
        # 三种可选 query 策略："original"（原版固定 100 个 query）、"topk"（按得分剪枝）、"linear"（线性层压缩）
        query_variant = getattr(cfg.MODEL.MASK_FORMER, "QUERY_VARIANT", "original")
        assert query_variant in ["original", "topk", "linear"], f"Unsupported QUERY_VARIANT: {query_variant}"
...
class MaskFormerModel_baseline(nn.Module):
    ...
    def forward(self, inputs):
        ...
        features = self.backbone(inputs) #输出不同尺寸的特征图（res2，res3，res4，res5）
        outputs = self.sem_seg_head(features) #feat5(6,768,16,16)
