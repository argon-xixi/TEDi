...
from .MSE.memory_encorder import VideoQueryMemoryModule
from .backbone.swin import D2SwinTransformer
from .pixel_decoder.msdeformattn import MSDeformAttnPixelDecoder
from .transformer_decoder.mask2former_transformer_decoder import MultiScaleMaskedTransformerDecoder

from .TCD.denoising_module import TCDdenoising
from utils.criterion_denoising import SetCriterion, Criterion
...
class MaskFormerModel_denoising_memorybank(nn.Module):
    def __init__(self, cfg):
        ...
        self.iter=0
        self.cfg=cfg
        self.denoising_decode = self.build_denoising_decode(cfg)
        ...
        if mem_cfg is None:
            ...
            self.num_denoising_frames = 3
        else:
            self.num_mem_frames = int(getattr(mem_cfg, "NUM_MEM_FRAMES", mem_cfg.get("NUM_MEM_FRAMES", 3)))
            self.num_denoising_frames = int(getattr(mem_cfg, "NUM_denoising_FRAMES", mem_cfg.get("NUM_denoising_FRAMES", 3)))
    ...
    def forward_infer(
        ...
        self.denoising_decode.eval()

        B, T, C_in, H, W = inputs.shape
        m = int(self.num_mem_frames)
        n = int(self.num_denoising_frames)
        if m <= 0 or n <= 0:
            raise ValueError(f"NUM_MEM_FRAMES(m) and NUM_denoising_FRAMES(n) must be > 0, got m={m}, n={n}")
        if T < m + n:
            raise ValueError(f"Need at least T >= m+n frames, got T={T}, m={m}, n={n}")

        mem_slice = slice(0, m)
        denoising_slice = slice(m, m + n)
        T_denoising = n
        ...
        frame_embds_denoising = frame_embds_all[:, :, denoising_slice, :]
        mask_feature_denoising = image_outputs['mask_features'][:, denoising_slice, :, :, :]
        ...
        if use_memory_bank:
            ...
            frame_embds = self.memory_encoder(
                frame_embds_all,
                mask_all,
                frame_embds_denoising,
                logits_all,
                proc_frame_ids=proc_frame_ids,
            )
        else:
            frame_embds = frame_embds_denoising

        outputs_ori = self.denoising_decode(
            frame_embds,
            mask_feature_denoising,
        ...
        if isinstance(outputs_ori, dict) and ('pred_logits' in outputs_ori) and ('pred_masks' in outputs_ori):
            ...
            if pl.dim() == 4:
                ...
                out['pred_logits'] = einops.rearrange(pl, '(b t) q c -> b t q c', t=T_denoising)
            else:
                out['pred_logits'] = pl
            ...
            if pm.dim() == 5:
                ...
                if pm.shape[2] == 1:
                    out['pred_masks'] = einops.rearrange(pm, '(b t) q () h w -> b t q h w', t=T_denoising)
                else:
    ...
    def build_denoising_decode(self, cfg):

        denoising_decoder = TCDdenoising(
            ...
            decoder_layer_num=cfg.MODEL.denoising_decode.DECODER_LAYERS,
    ...
    def forward(self, inputs, targets_ori, istrain, name_idx, epoch, i, name):
        ...
        # Context frames populate memory; later frames receive denoisinging supervision.
        mem_slice = slice(0, m)
        denoising_slice = slice(m, m + n)
        T_denoising = n
        output_denoising={}
        with torch.no_grad():
            ...
            output_denoising['mask_features']=image_outputs['mask_features'][:, denoising_slice, :, :, :]
            output_denoising['pred_logits']=image_outputs['pred_logits'][:, denoising_slice, :, :]
            output_denoising['pred_masks']=image_outputs['pred_masks'][:, :, denoising_slice, :, :]
            output_denoising['pred_embds']=image_outputs['pred_embds'][:, :, denoising_slice, :]
            if 'aux_outputs' in image_outputs:

                output_denoising['aux_outputs'] = []

                for k in range(len(image_outputs['aux_outputs'])):

                    aux_output = image_outputs['aux_outputs'][k]

                    output_denoising['aux_outputs'].append({
                        'pred_logits': aux_output['pred_logits'][:, denoising_slice, :, :],
                        'pred_masks': aux_output['pred_masks'][:, :, denoising_slice, :, :]
                    })
            del image_outputs
        ...
        outputs_ori, indices = self.denoising_decode(
            frame_embds,
            mask_feature_denoising,
            return_indices=True,
            resume=self.keep,
        ...
        output_denoising = self.reset_image_output_order(output_denoising, indices)
        targets_denoising_ori=targets_ori[:, denoising_slice, :, :]

        targets = self._get_targets(targets_denoising_ori)

        if self.iter < self.cfg.SOLVER.MAX_ITER // 2:
            output_denoising, outputs, targets = self.frame_decoder_loss_reshape(
                outputs_ori, targets, output_denoising=output_denoising
            )
        else:
            output_denoising, outputs, targets = self.frame_decoder_loss_reshape(
                outputs_ori, targets, output_denoising=None
            )
        ...
        losses, indice_dict = self.criterion(
            outputs,
            targets_denoising_ori.squeeze(0),
            matcher_outputs=output_denoising,
        )

        outputs_reshape = {}
        outputs_reshape['pred_logits'] = einops.rearrange(
            outputs['pred_logits'], ' (b t) q c -> b t q c', t=T_denoising
        )
        outputs_reshape['pred_masks'] = einops.rearrange(
            outputs['pred_masks'], ' (b t) q () h w -> b t q h w', t=T_denoising
        )
        ...
        if istrain:
            ...
            outputs_ori['pred_logits'] = einops.rearrange(
                outputs_ori['pred_logits'], ' (b t) q c -> b t q c', t=T_denoising
            )
            outputs_ori['pred_masks'] = einops.rearrange(
                outputs_ori['pred_masks'], ' (b t) q () h w -> b t q h w', t=T_denoising
            )
    ...
    def frame_decoder_loss_reshape(self, outputs, targets, output_denoising=None):
        ...
        if output_denoising is not None:
            output_denoising['pred_masks'] = einops.rearrange(output_denoising['pred_masks'], 'b q t h w -> (b t) q () h w')
            output_denoising['pred_logits'] = einops.rearrange(output_denoising['pred_logits'], 'b t q c -> (b t) q c')
        ...
        return output_denoising, outputs, gt_instances
